...
static void *extend_heap(size_t words) /* 새 가용 블록으로 힙을 확장하는 함수 */
{
    ...
    size = (words % 2) ? (words+1) * WSIZE : words * WSIZE; // 홀수면 true, 작수면 false
    if ((long)(bp = mem_sbrk(size)) == -1) { // mem_sbrk가 어디서 나온거지? 내장함수인가
        return NULL;
    }
...
{ // 원래 써있던 코드
    ...
    if (size <= DSIZE) {
        asize = 2*DSIZE; // adjust size : 조정된 사이즈. 2*8 = 16
    }
...
// }
        ...
        // for (bp = next_ptr ; GET_SIZE(HDRP(next_ptr)) > 0 ; next_ptr = NEXT_BLKP(next_ptr)) {
...
static void *coalesce(void *ptr)
{
    ...
    // next fit
    // if ((next_ptr > (char*)ptr) && (next_ptr < NEXT_BLKP(ptr)))
        // next_ptr = ptr;
    next_ptr = ptr; // next fit을 쓰기 위해 저장해준다 (find fit에서 설정)
    return ptr;
}
