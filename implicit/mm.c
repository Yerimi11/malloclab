...
void *mm_realloc(void *bp, size_t size) { /* 이미 할당한 공간의 크기를 바꿀 때 사용(재할당), 뒤에까지 다 free하는 방식 */
    ...
    // new_size가 old_size보다 크면 사이즈 변경
        ...
        size_t next_alloc = GET_ALLOC(HDRP(NEXT_BLKP(bp))); // 다음 블록의 헤더의 할당값(0, 1)을 받아옴 (다음 블럭 할당 여부)
        size_t current_size = old_size + GET_SIZE(HDRP(NEXT_BLKP(bp))); // 이전블럭+다음블럭 = 현재블럭
